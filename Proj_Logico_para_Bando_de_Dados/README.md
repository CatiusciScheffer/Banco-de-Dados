...
###**Descrição de como cumpri o desafio**

1. Fiz e refinei o modelo conceitual conforme imagem abaixo:
...
```
![Produto mais vendido](./img/prod_mais_vendido.png)
...
ORDER BY Saldo;
```
![Saldo de estoque](./img/saldo_estoque.png)

######CALCULANDO TOTAL DO PEDIDO (COMPRAS + FRETE)
```sql
SELECT 
    CONCAT(cliente_Pnome, ' ', cliente_SegundoNome, ' ', cliente_Sobrenome) 'Cliente',
    ...
    ROUND((pedprod_Quantidade * pedprod_VLR_unitario),2) AS 'Total',
    ROUND(((pedprod_Quantidade * pedprod_VLR_unitario) + pedido_VrlFrete),2) AS 'Total+Frete'
FROM
    Produto_Pedido pp 
INNER JOIN 
...
```
![Valor total pedido](./img/valor_total_pedido.png)

######TOTAL DE PEDIDOS DE 04/2022
```sql
SELECT 
    CONCAT(cliente_Pnome, ' ', cliente_SegundoNome, ' ', cliente_Sobrenome) 'Cliente',
    pedprod_dataPedido 'Data Pedido',
...
```
![Total de pedidos em 042022](./img/pedidos042022.png)
...
```
![Quantidade de pedido por cliente](./img/total_pedido_cliente.png)
...
principais informações de uma compra, desde o dado do cliente até as informações de preço, frete e situação do pedido e do estoque do produto*/
